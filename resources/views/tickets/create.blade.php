<x-app-layout>
    ...
    <x-slot name="header">
        ...
        <form action="{{ route('tickets.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="flex  items-start content-center mb-6">
                @foreach ($labels as $label)
                    <div class="flex justify-evenly items-center ml-3 h-5">
                        <input id={{ $label->name }} type="checkbox" value="{{ $label->name }}"
                            name="label[{{ $label->id }}][]"
            ...
            <label class="block mb-2 mt-3 text-sm font-medium text-gray-900 dark:text-white" for="multiple_files">Upload
